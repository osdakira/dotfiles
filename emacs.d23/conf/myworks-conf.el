...
(defun change-frame-background-color-by-project()
  ...
  (when (not (equal buffer-file-name nil))
    (cond 
     ((string-match "genju" buffer-file-name)
      (set-face-foreground 'modeline "#F000F0"))
     ((string-match "seishun" buffer-file-name)
      (set-face-foreground 'modeline "#0000FF"))
     ;; ((string-match "otogi" buffer-file-name)
     ;;  (set-face-foreground 'modeline "#FFF0FF"))
...
(add-hook 'elscreen-goto-hook 'change-frame-background-color-by-project)
(add-hook 'anything-after-action-hook 'change-frame-background-color-by-project)
(add-hook 'find-file-hook 'change-frame-background-color-by-project)
