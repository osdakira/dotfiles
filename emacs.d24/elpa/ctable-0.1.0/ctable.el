...
Because this function iterates over the entire region, 
it takes much time to search the cell in the big table.
If FUNC returns the symbol `break', this function breaks the loop immediately.
 (Just performance short-cut)"
  (loop with pos = (ctbl:dest-point-min dest)
        with end = (ctbl:dest-point-max dest)
        for next = (next-single-property-change pos 'ctbl:cell-id nil end)
        for text-id = (and next (ctbl:cursor-to-cell next))
        ...
        (if (and text-id (equal row-id (car text-id)))
            (let ((cend (next-single-property-change
                         next 'ctbl:cell-id nil end)) ret)
              (when (eq 'break (funcall func text-id next cend))
                (return))))
        (setq pos next)))
